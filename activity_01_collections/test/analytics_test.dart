...
void main() {
  ...
  group('revenueByPlatform', () {
    test('calculates total revenue correctly for each platform', () {
      ...
      expect(result['iOS'], closeTo(79.96, 0.01));
      expect(result['Android'], closeTo(49.97, 0.01));
    });

    test('returns empty map for events with no purchases', () {
      final noPurchases = events.where((e) => e.eventType != 'purchase').toList();
  ...
  group('activeUsersByHour', () {
    test('counts unique users per hour correctly', () {
      ...
      expect(result[10], equals(4)); // user_234, user_567, user_456, user_123
      expect(result[11], equals(3)); // user_890, user_345, user_234
    });
  ...
  group('errorRateByPlatform', () {
    test('calculates error rates correctly', () {
      ...
      // iOS: 2 errors out of 16 events = 0.125
      expect(result['iOS'], closeTo(0.125, 0.01));
      
      // Android: 2 errors out of 14 events ≈ 0.143
      expect(result['Android'], closeTo(0.143, 0.01));
    });
  ...
  group('topUsers', () {
    ...
    test('handles n larger than number of users', () {
      ...
      expect(result.length, lessThanOrEqualTo(events.map((e) => e.userId).toSet().length));
    });
    ...
    test('handles n = 1', () {
      ...
      expect(result[0].$2, greaterThanOrEqualTo(4)); // Top user has at least 4 events
    });
  });
      ...
      for (final userEvents in result.values) {
        for (var i = 0; i < userEvents.length - 1; i++) {
          expect(
            ...
            userEvents[i].timestamp.isAtSameMomentAs(userEvents[i + 1].timestamp),
            isTrue,
